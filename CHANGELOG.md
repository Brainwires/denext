...
- **Client hydration regression (never published — introduced and fixed between rc.6 and rc.7).** The server renderers' shared base class imported `isPostpone` from `runtime/prerender.ts`, whose `node:async_hooks` import then rode along into every production client chunk; the strict CSP refused the `node:` script, so nothing hydrated — with no console error, since the bundle still parsed. The postpone signal now lives in `src/runtime/postpone.ts` (no `node:*` import), and `tests/integration/build-smoke.test.ts` gained a tripwire that fails the build if any client file imports a Node builtin.
...
- **Class-component runtime leaked into every `classComponents: false` compat client bundle (never published — introduced by the reconciler decomposition after rc.6).** `renderComponent`'s split put the class branch behind `isClassComponent(...)` alone, so `renderClassFiber` — and with it the whole class runtime — stayed referenced when the build flag folded to `false`; the OFF client bundle grew ~4 KB and the "zero cost when off" guarantee broke (caught by the `next-compat-class` e2e). The branch is gated on `__DENEXT_CLASS_COMPONENTS__` first again.
