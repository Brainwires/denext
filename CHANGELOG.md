...
- **Client hydration regression (unreleased; introduced by the renderer-sharing refactor).** The server renderers' shared base class imported `isPostpone` from `runtime/prerender.ts`, whose `node:async_hooks` import then rode along into every production client chunk; the strict CSP refused the `node:` script, so nothing hydrated — with no console error, since the bundle still parsed. The postpone signal now lives in `src/runtime/postpone.ts` (no `node:*` import), and `tests/integration/build-smoke.test.ts` gained a tripwire that fails the build if any client file imports a Node builtin.
...
- **`experimental.streaming` / `experimental.live` legacy aliases.** Both fields have been top-level (`streaming`, `live`) since 1.4; the loose-cast fallbacks that still read the old keys are gone. Setting either legacy key now does nothing except emit a dev warning naming the top-level field — move the value up.
