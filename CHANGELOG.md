...
- **`denext migrate --from remix` — assisted Remix migration.** The first migration path that physically **transforms the route tree** rather than only writing config (auto-detected from `@remix-run/*` deps / `remix.config.*` / `app/root.tsx`+`app/routes/`, and short-circuiting the Vite-SPA detector so Remix-Vite isn't miscaptured). It restructures `app/routes/*` (flat-file + dot-nested, and the `route.tsx` folder form) into denext folder-per-segment `app/**/page.tsx`+`layout.tsx` — `$param` → `[param]`, `$` splat → `[...splat]`, `_index` → the segment page, pathless `_x` → a `(x)` route group, trailing-`_` break-out flattened + flagged; converts `app/root.tsx` → `app/layout.tsx` (strips `<Meta/>`/`<Links/>`/`<Scripts/>`/`<ScrollRestoration/>`, `<Outlet/>` → `{children}`, injects the `{ children }` prop); deletes `entry.{server,client}.*`; and writes the denext config (import map + tasks + `.gitignore` + `.vscode`, react→denext, `@remix-run/*` dropped). The **data-model inversion is scaffolded, not magic** (assisted): a `loader` is inlined into an `async` Server Component (`useLoaderData()` → `await loader()`), an `action` is marked `"use server"`, `@remix-run/react` imports are split (`Link`/`NavLink`/`useParams` → denext, `<Link to>` → `<Link href>`; the rest flagged), `@remix-run/node` `json()` is unwrapped to a plain value and `redirect` mapped to denext — each with a `TODO(denext migrate)` banner, and the run reports how many loaders/actions and structural edges need review. `src/build/remix-migrate.ts` (new), `src/build/migrate.ts`, `src/cli/commands/migrate.ts`, `src/build/codemod.ts`.
