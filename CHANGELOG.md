...
- **Dev server black box + live MCP tools (`denext_list_routes`, `denext_dev_logs`).** The dev server now keeps a bounded in-memory recorder of runtime signal — server-side render/build/type-check errors (with the codeframe the overlay already builds) plus the **browser's** `console.error`/`warn` and uncaught errors/rejections, shipped back over a new same-origin-gated `POST /_denext/dev-log` by the dev-reload client that's already on every dev page — and exposes it at `GET /_denext/dev-state`. On boot it publishes its address to `.denext/dev.json` (removed on drain) so a reader can find it. Two new MCP tools use this: `denext_dev_logs` reads the **running** dev server's recent errors + browser console (so an agent sees what actually broke at runtime, not just static source), and `denext_list_routes` lists an app's pages + API routes with their dynamic params (no dev server needed). Both endpoints reuse the reload stream's cross-origin gate; a server-side reader (no `Sec-Fetch-Site`) is allowed, a cross-site page is refused. `src/build/dev-events.ts` (new — the ring buffer), `src/build/dev-server.ts`, `src/mcp/dev-client.ts` (new), `src/mcp/tools.ts`.
