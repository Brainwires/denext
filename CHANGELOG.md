...
- **Remix compat surface: closed 9 of the 13 parity gaps.** After bringing `src/compat/remix/` under the signature-parity gate, burned down the trivial deviations vs `@remix-run/react` / `@remix-run/node`: `isCookie`/`createSession` are now exported (they existed privately), `isSession` is added, `useSearchParams(defaultInit)` honors default params for keys absent from the URL, and `defer(data, init?)` / `useFetcher({ key })` / `useHref`/`useFormAction`/`useResolvedPath` accept their Remix optional arguments (a supplied `useFetcher` key is used for the registry; the `relative` path-resolution option and `defer`'s `ResponseInit` are accepted for source-compat — denext resolves route-absolute and threads deferred data over Flight). The 4 remaining gaps (`data()` and `replace()` on both packages) stay tracked in the known-gaps ledger — they need loader/history support, not a signature tweak. `src/compat/remix/{client,server}.ts`.
