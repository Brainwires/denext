# denext — Product & Go-to-Market Strategy

> Status: internal strategy document. Split out of ROADMAP.md on 2026-08-24 so the
> roadmap can track only remaining engineering; this file is the permanent home for
> positioning, objections, the phased adoption plan, launch, and risk. It is **not** a
> roadmap and is not slated for removal — it is the standing answer to "why anyone
> adopts denext, and in what order we earn that."
>
> Companion docs: [MISSION.md](./MISSION.md) (the five superiority pillars),
> [FEATURES.md](./FEATURES.md) (what's shipped), [ROADMAP.md](./ROADMAP.md) (the
> remaining zero-npm / ecosystem engineering backlog), [ROADMAP-2.0.md](./ROADMAP-2.0.md)
> (the 2.0 DX release), and [KNOWN-LIMITATIONS.md](./KNOWN-LIMITATIONS.md) (the honest gaps).
>
> Grounded in four parallel research passes (Aug 2026); see the appendix.
...
> The pending **engineering** backlog (DevTools depth, build-time deps → JSR, the
> RSC render-mode panel) lives in [ROADMAP.md](./ROADMAP.md), not here.
...
- **The "cloned the parts people hate" trap (#8):** make caching/RSC **observable**
  (the glass-box devtools — see the engineering backlog in [ROADMAP.md](./ROADMAP.md)) —
  same semantics, better visibility, with `<Live>` boundaries lighting up. A genuine edge.
- **Longevity (#5):** see §A5.
