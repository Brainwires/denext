...
> couple commits out). This is the product/go-to-market roadmap that sits *after*
> the engineering checklist in [ROADMAP-1.0.md](./ROADMAP-1.0.md). ROADMAP-1.0
> answers "what code ships in 1.0"; this answers "why anyone adopts it, and in
> what order we earn that."
>
...
- **"Next.js compatibility on Deno" is a race we lose to real Next.js.** As of
  ...
  reader's first reaction to a compatibility pitch is *"why a clone when I can
  run the real thing on Deno with the real ecosystem?"* Compatibility, framed as
...
- **The one thing neither real-Next-on-Deno nor Fresh can offer is a zero-npm
  ...
  Preact + npm-compat. denext's own-React reimplementation is the *only* reason
...
- **Compatibility is the *on-ramp*, not the pitch.** Compatibility-as-migration
  ...
  it — as the thing that makes *trying* denext free — while the wedge that makes
  people *want* to is the auditable, tiny, dependency-free output.
...
- **No deploy recipes** (no Dockerfile / Deno Deploy walkthrough / self-host
  guide; a concurrency ceiling is *required* but left to the user).
...
- **Migration is effectively greenfield** (no codemod; per-page manual wiring;
  the "90 pages probed clean" figure is a module-*load* probe, not a rendered
...
|---|---|---|---|
| 1 | It's a React *reimplementation*, not React (trust) | Fatal | Parity-test matrix + honesty docs (Phase 3) |
| 2 | Deno already runs *real* Next.js — why a clone? | Fatal | The positioning pivot §1 + zero-npm demo (Phase 1) |
...
| 8 | We cloned the parts people hate (RSC/caching) | Med | Make caching/RSC *observable* (Phase 3) |
...
Note the trap: denext structurally resembles the frameworks that *failed*
...
lucide, no secrets: denext's *claimed* sweet spot) — through
...
bundle and reached SSR), but an *unmodified* clone stops at the **build** stage,
...
1. **`sloppy-imports` not propagated (the #1 build blocker).** Next apps use
   ...
   config (`src/build/bundle.ts` `prepareConfig`) *drops the app's `unstable`
   field*. Fix: add the flag to child `run` + `info` + `bundle` and preserve
...
line is *"an automated converter plus a short, known list of compat fixes away."*
...
2. **Reconcile [ROADMAP-1.0.md](./ROADMAP-1.0.md)** — it still lists async
   transitions, `useInsertionEffect`, Offscreen, RSC nav, and Fast Refresh as
   *todo*; they've shipped. A reader who checks will think it's unfinished.
...
The wedge only lands if it's *shown*. Lead with one felt-pain number, backed by
...
1. **The supply-chain / single-binary demo.** `deno compile` / `deno desktop` →
   ...
   **size + dependency-count delta is the proof point** — `deno desktop` gives
   competitors a single-binary story too, so the *delta*, not the capability, is
...
4. **Least-privilege by default:** ship and document a tight `deno run
   --allow-...` profile for `denext start`. The permission model only *means*
...
| **P0** | Deploy recipes | Dockerfile + Deno Deploy walkthrough + self-host/systemd recipe. Ship the *required* concurrency ceiling as a documented default, not homework. |
...
| **P1** | Data layer | Prove *one* real Postgres path (Drizzle or native driver) with an example; position Deno KV as an app data store, not just cache. |
...
These never fully disappear; the goal is to stop them being *instant* dismissals.

- **Trust in the reimplementation (#1):** grow parity tests *named after* the
...
Technical quality is table stakes — every framework that *failed* was
...
2. **Answer "why not just use real Next?" on the first screen** — the guaranteed
   top comment. One-line rebuttal: *familiar Next API, ~10× smaller output, zero
   npm runtime deps, native Deno with no unstable flags and no Vercel lock-in.*
...
5. **Be the evangelist.** Every winner had a visible, responsive
   ...
   not. Talks, build-in-public, fast issue responses *are* the marketing.
...
- **De-risk structurally.** MIT (forkable if you vanish); **zero npm runtime
  deps is itself a bus-factor *reduction*** — fewer external points of failure,
...
- **Cap-the-ceiling counter.** Frame denext as riding Deno-2's growth and its
  npm bridge — *not* betting on Deno replacing Node — so a smaller runtime
...
- Don't lead with Next-security-FUD — it reads as fearmongering and triggers
  dismissal. Zero-npm is a *positive* architecture story, not an attack.
...
1. *Why devs leave/stay with Next.js* — the loud complaints (complexity, caching,
...
2. *Deno platform + Fresh landscape* — zero-npm is the only claim unique vs. both
   real-Next-on-Deno and Fresh; prove it via `deno compile`/`deno desktop`.
   Aleph (the prior "Next for Deno") archived July 2025.
3. *denext gap inventory* — auth, DB, deploy recipes, app-testing are the
   adoption blockers; core rendering/compat/security/types are mature.
4. *What drives adoption* — one sharp felt-pain number; a migration on-ramp; a
