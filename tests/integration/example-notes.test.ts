...
async function stepHomeFeed(client: TestClient): Promise<void> {
...
async function stepGateSignedOut(client: TestClient): Promise<void> {
  const res = await client.get("/notes");
  assertEquals(res.status, 307);
  assertStringIncludes(res.location ?? "", "/login");
}

async function stepBadCredentials(): Promise<void> {
...
async function stepGateSignedIn(client: TestClient): Promise<void> {
  const res = await client.get("/notes");
  assertEquals(res.status, 200);
...
async function stepCreateNote(client: TestClient): Promise<void> {
...
async function stepEditForeignNote(client: TestClient): Promise<void> {
...
async function stepMissingNote(client: TestClient): Promise<void> {
  const res = await client.get("/notes/9999/edit");
  assertEquals(res.status, 404);
  assertStringIncludes(res.text, "Not found");
}

async function stepSignOut(client: TestClient): Promise<void> {
  const page = await client.get("/notes");
