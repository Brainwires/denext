...
Deno.test({
  ...
  await t.step(
    ...
    async () => {
      ...
      // The CLI may print download/progress lines before the module; compare from its header.
      const start = emitted.out.indexOf("// Generated by `denext openapi types`");
      assert(start >= 0, emitted.out);
      assertEquals(
        emitted.out.slice(start).trimEnd(),
        committed.trimEnd(),
