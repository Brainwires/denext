...
Deno.test({
  ...
  await t.step(
    "`denext openapi types` regenerates the committed lib/api-types.gen.ts byte for byte",
    async () => {
      const committed = await Deno.readTextFile(EXAMPLE + "/lib/api-types.gen.ts");
      const emitted = await runDeno(
        ["run", "-A", CLI, "openapi", "types"],
        EXAMPLE,
        BUILD_TIMEOUT_MS,
      );
      assert(emitted.ok, "denext openapi types failed:\n" + emitted.out);
      // The module is what reaches STDOUT — the CLI's stderr (a download line, the denextAuth
      // `canonicalOrigin` warning the example's config provokes) is advice, not output, and
      // comparing both streams together is what kept this red for four nights. The CLI may
      // still print progress lines to stdout before the module; compare from its header.
      const start = emitted.stdout.indexOf("// Generated by `denext openapi types`");
      assert(start >= 0, emitted.out);
      assertEquals(
        emitted.stdout.slice(start).trimEnd(),
        committed.trimEnd(),
        "lib/api-types.gen.ts is stale — run `deno task openapi:types` in examples/openapi",
      );
    },
  );
