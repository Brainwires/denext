...
async function stepLoadAndOnly(page: Page): Promise<void> {
...
async function stepVisible(page: Page): Promise<void> {
...
async function stepInteraction(page: Page): Promise<void> {
...
async function stepMedia(page: Page): Promise<void> {
...
function stepHydrationLogs(logs: string[]): void {
...
Deno.test({
  ...
  try {
    await t.step(
      "server HTML carves all six islands; client:only has no SSR body",
    ...
    await t.step("client:load and client:only hydrate immediately", () => stepLoadAndOnly(page));
    await t.step("client:idle hydrates on idle without interaction", async () => {
      await page.waitForFunction(hydrated("idle"));
    });
    await t.step("client:visible stays inert until scrolled into view", () => stepVisible(page));
    await t.step(
      "client:interaction stays inert until first interaction, then replays it",
      () => stepInteraction(page),
    );
    await t.step(
      "client:media hydrates iff its query matches the viewport",
      () => stepMedia(page),
    );
    await t.step(
      "each island logged its own hydration; no hydration mismatch",
      () => stepHydrationLogs(logs),
    );
  } finally {
    await browser.close();
    await server.close();
