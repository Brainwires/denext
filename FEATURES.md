...
- **Unified CLI** — a real command framework (declarative flags, uniform global flags
  ...
  `add`/`remove`/`update`, `plugin add`/`plugin remove` (installs/uninstalls a plugin dep
  **and** wires/unwires it in `denext.config.ts`), `doctor`/`info` (`doctor` supersedes `probe`, kept as an alias),
