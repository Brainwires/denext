...
| `experimental.optimizePackageImports` (barrel-import rewriting)                                                                                                                 | ✅ top-level `optimizePackageImports` in `denext.config.ts` (`denext migrate` maps it; a default list is on — `"!pkg"` excludes, `false` disables; see [Bundling](/docs/bundling#barrel-imports-optimizepackageimports)) |
