...
can't drift. It is deliberately shallow — property names, kinds, and each
field's description, i.e. what the runtime validator enforces. Point an editor
or a config linter at it for autocomplete on a `.js` config; a `.ts` config
gets the same from `satisfies DenextConfig`.
