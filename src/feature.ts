 ...
 *   // …only reaches the bundle when `features.NEW_CHECKOUT` is true in denext.config.ts
 * }
 * ```
 *
 * `feature()` always returns the configured value — the server and every client bundle are
 * seeded with the top-level `features` map of `denext.config.ts`. At build time a
 * `feature("KEY")` call with a string-literal KEY is additionally FOLDED to `true`/`false`
 * where the build can, so the
 ...
 * in `features`. Server rendering and unbundled runs read the same seeded map (see
 * src/runtime/feature-flags.ts).
 *
 * @module
 */
 ...
 * `features`) so the branch is dead-code eliminated; otherwise it reads the build-seeded
 * flag map at runtime, defaulting to `false` for any unset flag.
 *
 * @param name The flag name, matching a key in `features` in `denext.config.ts`.
 */
