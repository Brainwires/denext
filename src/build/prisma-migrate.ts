...
// (bundle the compat → install → `prisma generate` → `prisma migrate deploy`/`db push`). After setup the app
...
/** Write the one-shot setup script (bundle compat → install → generate → migrate deploy / db push). */
