...
/** A denext element (React's `ReactElement`). The 2nd param (element `type`) is
 * accepted for `@types/react` shape compatibility; denext keys elements on `VNode`. */
export type ReactElement<P = any, _T = any> = VNode & { props: P };
...
export type ElementRef<T> = T extends keyof IntrinsicElementInstances ? IntrinsicElementInstances[T]
  : unknown;
/** React 19's alias for {@linkcode ElementRef}. */
export type ComponentRef<T> = ElementRef<T>;

// --- Hooks & state -----------------------------------------------------------

/** `useState`/`useReducer` dispatcher. */
export type Dispatch<A> = (value: A) => void;
/** A dispatcher for a reducer that takes no action. */
export type DispatchWithoutAction = () => void;
/** The updater passed to a state setter (a value or a function of the previous). */
export type SetStateAction<S> = S | ((prev: S) => S);
/** A `useReducer` reducer. */
export type Reducer<S, A> = (prevState: S, action: A) => S;
/** A reducer that takes no action. */
export type ReducerWithoutAction<S> = (prevState: S) => S;
/** The state type of a {@linkcode Reducer}. */
export type ReducerState<R> = R extends Reducer<infer S, any> ? S : never;
/** The action type of a {@linkcode Reducer}. */
export type ReducerAction<R> = R extends Reducer<any, infer A> ? A : never;
/** A `useEffect` cleanup-returning callback. */
...
/** Anything usable as a JSX tag: an intrinsic tag name or a component. */
export type ElementType<P = any> = string | ComponentType<P>;
/** Legacy: a single renderable child. */
export type ReactChild = ReactElement | string | number;
/** Legacy: renderable text. */
export type ReactText = string | number;

// --- Context -----------------------------------------------------------------

/** A context provider component (`<Ctx.Provider value>`). */
export interface Provider<T> {
  (props: { value: T; children?: ReactNode }): ReactElement | null;
}
/** A context consumer component (render-prop). */
export interface Consumer<T> {
  (props: { children: (value: T) => ReactNode }): ReactElement | null;
}
/** Props of a {@linkcode Provider}. */
export interface ProviderProps<T> {
  /** The value to provide to descendants. */
  value: T;
  /** The subtree that reads the value. */
  children?: ReactNode;
}
/** Props of a {@linkcode Consumer}. */
...
export interface CompositionEvent<T = Element>
  ...
  /** The characters generated by the composition. */
  data: string;
}
/** A drag event. */
...
/** Handler for an IME composition event. */
...
/** Map of SVG tag names to their component factories (permissive; used by icon libs). */
export interface ReactSVG {
  /** An SVG tag name → its component factory. */
  [tag: string]: (props: SVGProps<SVGElement>) => ReactElement | null;
}
/** React's `HTMLProps`. */
