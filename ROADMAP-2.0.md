...
7. **Advertise last.** No launch push until the success criteria in §11 are met.
...
## 9. Pillar VII — Docs & Learnability
...
## 11. Success criteria ("deliver before we advertise")
...
## 13. Relationship to 1.x & release mechanics
...
- **A decisive major cut.** First major bump: update the version in both `deno.json` **and** `mod.ts`;
  land pillars incrementally on `development`; cut `v2.0.0` (tag-triggered JSR publish) once §11 is met —
  not a long alpha.
