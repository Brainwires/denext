...
2. **`expo-*` API shims**, aliased the same way `react` → denext is. **Done:** `denext/expo/*`,
   one module per package (34, every `expo-*` dependency of T3's `apps/mobile` except
   `expo-sqlite`), listed with status and omissions in `src/expo/manifest.ts` (`EXPO_SHIMS`,
...
| `expo-widgets`           | stub    | inert factories                                                 | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
screen loads the environment (`expo-secure-store` had blocked it). Still needed outside the
shims: an `expo-sqlite` stub (its real web build never opens the database without
cross-origin isolation, which leaves every route blank), a one-line
`Appearance.setColorScheme` polyfill (react-native-web lacks it), and the app's stubs for
non-Expo native modules (Nitro, T3's native terminal/markdown/review-diff, `@expo/ui`).
