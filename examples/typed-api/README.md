...
- **`defineApi`** ([`app/api/todos/route.ts`](./app/api/todos/route.ts)) — route
  ...
  (a 60-line hand-rolled one in [`lib/schema.ts`](./lib/schema.ts);
...
- **`createApiClient()` / `useApi`** ([`app/todos.tsx`](./app/todos.tsx)) —
...
```sh
curl -s localhost:8000/api/todos | jq
curl -s -X POST localhost:8000/api/todos -H 'content-type: application/json' -d '{"title":5}' | jq
