...
/** The readout: frame count, smoothed fps, and started vs committed transition renders. */
function statsText(
  frames: number,
  fps: number,
  started: number,
  ...
  const dropped = started > committed ? `   (interrupted/coalesced ${started - committed})` : "";
  return `frames: ${frames}   ~fps: ${fps.toFixed(0)}\n` +
    `transition renders started:   ${started}\n` +
    `transition renders committed: ${committed}${dropped}`;
}
...
export default function ConcurrencyPage() {
  ...
  const { spinnerRef, statsRef, started } = useFrameProbe(count);
  ...
  return (
    <main style="font-family:system-ui,sans-serif;max-width:52rem;margin:2rem auto;padding:0 1rem">
      ...
      />
      <UrgentInput value={input} onInput={setInput} />
...
/**
 * The animation + live probe: a rAF loop that spins the spinner and rewrites the stats
 * block via direct DOM (never React state), so it keeps ticking between transition
 * slices and reports the truth — if the spinner stalls, the main thread was blocked.
 * `started` counts transition renders kicked off; `committed` counts heavy-grid commits
 * that actually landed (a passive effect per committed `count`).
 */
function useFrameProbe(count: number) {
  ...
  return { spinnerRef, statsRef, started };
}
...
/** The blocking-mode toggle, the grid-size slider and the render status line. */
function GridControls(props: {
  blocking: boolean;
  onBlocking: (v: boolean) => void;
  slider: number;
  ...
  return (
    <>
      <label style="display:block;margin:.75rem 0">
        <input
          type="checkbox"
          checked={props.blocking}
          onChange={(e: Event) => props.onBlocking((e.currentTarget as HTMLInputElement).checked)}
        />{" "}
      ...
      <input
        ...
        step={250}
        value={props.slider}
        onInput={props.onSlider}
        style="width:100%"
      />
      <p style="min-height:1.5em;color:#6d28d9">{props.status}</p>
    </>
  );
}
