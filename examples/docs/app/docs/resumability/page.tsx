...
export default function Resumability() {
  return (
    ...
    >
      <h2>What it is</h2>
      <p>
        There are two ways to make server-rendered HTML interactive in the browser:
      </p>
      <ul>
        <li>
          <strong>Hydration</strong>{" "}
          (React, Next.js). On load, the client re-executes the whole component tree to rebuild
          state and attach event handlers over the existing DOM. The cost scales with how much
          interactive UI the page has — and it is paid up front, whether or not the visitor ever
          touches any of it.
        </li>
        <li>
          <strong>Resumption</strong>{" "}
          (denext). The client attaches nothing on load. It adopts the server DOM as-is and runs a
          component only when that part of the page is first used — then replays the triggering
          event to the now-live handler. Time-to-interactive stays flat no matter how large the page
          grows.
        </li>
      </ul>
      <Callout kind="note">
        ...
        <strong>Marko</strong>). denext&#39;s contribution is running it behind the ordinary{" "}
        <strong>React</strong>{" "}
        programming model: your components stay plain React, and they resume. That combination is
        what React and Next.js cannot offer, since their hydration is defined as re-running
        components — so this is resumability you can adopt without leaving React.
      </Callout>
...
<button onClick={onExport}>Export</button>`}
      ...
      <p>
        Resumable mode picks each island wake moment automatically from what it does: an island with
        only event handlers waits for the first interaction (maximal laziness), while one that runs
        an effect (a clock, a subscription) hydrates when the main thread is idle — so it ticks
...
<Widget client:load />       {/* hydrate immediately, but still per-island */}`}
      ...
      <p>
        Resumability pays off wherever up-front hydration is the bottleneck. It is opt-in per route,
        so turn it on where it helps and leave the rest of your app on normal hydration.
      </p>
      <ul>
        <li>
          <strong>Large, interactive pages</strong>{" "}
          — dashboards, editors, feeds, data tables with many widgets. Hydration wakes all of them
          on load; resumption wakes only the ones a visitor touches, so time-to-interactive stays
          flat as the page grows instead of climbing with every widget you add.
        </li>
        <li>
          <strong>Content with sparse interactivity</strong>{" "}
          — long articles or marketing pages with a few interactive pieces. Ship the HTML instantly
          and pay for a widget only if the reader actually uses it.
        </li>
        <li>
          <strong>Slow devices and cold loads</strong>{" "}
          — the hydration pass is exactly the main-thread work that makes a fresh page feel janky on
          a phone. Removing it is the most direct win for first-load responsiveness.
        </li>
      </ul>
      <p>
        When <em>not</em>{" "}
        to bother: a small or mostly-static page will not notice the difference, and it does add a
        few moving parts (a per-island runtime chunk, the delegated listener). Reach for it when the
        interactive surface is genuinely large or the audience is latency-sensitive — not by default
        on every route.
      </p>
